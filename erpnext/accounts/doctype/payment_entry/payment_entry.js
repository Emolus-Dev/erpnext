...
frappe.ui.form.on("Payment Entry", {
	...
	paid_from_account_currency: function (frm) {
		...
		frm.events.recalculate_payment_entry_currency_fields(frm);
	},
	...
	recalculate_payment_entry_currency_fields: function (frm) {
		if (!frm.doc.company || !frm.doc.paid_from_account_currency || !frm.doc.paid_to_account_currency) {
			return;
		}

		frappe.call({
			method: "erpnext.accounts.doctype.payment_entry.payment_entry.recalculate_payment_entry_currency_amounts",
			args: {
				company: frm.doc.company,
				paid_from_account_currency: frm.doc.paid_from_account_currency,
				paid_to_account_currency: frm.doc.paid_to_account_currency,
				...
				base_received_amount: frm.doc.base_received_amount,
				posting_date: frm.doc.posting_date,
			},
			callback: function (r, rt) {
				if (r.message) {
					frm.events.apply_payment_entry_currency_recalculation(frm, r.message);
				}
			},
		});
	},

	apply_payment_entry_currency_recalculation: function (frm, values) {
		frm._applying_payment_entry_currency_recalculation = true;
		frm.set_value(values).then(() => {
			frm._applying_payment_entry_currency_recalculation = false;
	...
	source_exchange_rate: function (frm) {
		if (frm._applying_payment_entry_currency_recalculation) {
			return;
		}
